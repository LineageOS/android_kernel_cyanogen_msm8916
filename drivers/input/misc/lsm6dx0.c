...
enum hrtimer_restart poll_function_read_acc(struct hrtimer *timer)
{
	...
	if (hrtimer_start(&stat->hr_timer_acc, stat->ktime_acc,
			HRTIMER_MODE_REL)) {
		dev_err(&stat->client->dev, "failed to start timer\n");
	}
...
enum hrtimer_restart poll_function_read_gyr(struct hrtimer *timer)
{
	...
	if (hrtimer_start(&stat->hr_timer_gyr, stat->ktime_gyr,
			HRTIMER_MODE_REL)) {
		dev_err(&stat->client->dev, "failed to start timer\n");
	}
