...
static int kionix_accel_vdd_power_init(struct kionix_accel_driver *data, bool on);
static int kionix_accel_vdd_power_ctl(struct kionix_accel_driver *data, bool on);
...
static int kionix_accel_grp4_standby(struct kionix_accel_driver *acceld)
{
	...
	/* hymost: DO NOT use cancel_delayed_work_sync*/
	if(acceld->accel_drdy == 0)
...
static int kionix_accel_enable(struct kionix_accel_driver *acceld)
{
	...
	printk("%s \n",__func__);
	kionix_accel_vdd_power_ctl(acceld, true);
...
exit:
	...
	kionix_accel_vdd_power_ctl(acceld, false);
...
static int kionix_accel_enable_set(struct sensors_classdev *sensors_cdev,
		unsigned int enable)
{
...
static int kionix_accel_poll_delay_set(struct sensors_classdev *sensors_cdev,
		unsigned int delay_ms)
{
	 ...
	 */
	acceld->poll_interval = max(delay_ms, acceld->accel_pdata.min_interval);
...
static int kionix_accel_vdd_power_ctl(struct kionix_accel_driver *data, bool on)
{
	...
	if (!on) {
		...
		if (ret) {
			...
			ret_tmp = regulator_enable(data->vdd);
			return ret;
		}
				...
				struct kionix_accel_platform_data *accel_pdata)
{
	...
	rc = of_property_read_u32(np, "kionix,accel-direction", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read accel-direction\n");
		return rc;
	} else {
		accel_pdata->accel_direction = (u8)temp_val;
		printk("%s accel_direction :%d \n",__func__,accel_pdata->accel_direction);
	}

	rc = of_property_read_u32(np, "kionix,accel-irq-use-drdy", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read accel-irq-use-drdy\n");
		return rc;
	} else {
		accel_pdata->accel_irq_use_drdy = temp_val;
		printk("%s accel_irq_use_drdy :%d \n",__func__,accel_pdata->accel_irq_use_drdy);
	}
	...
	rc = of_property_read_u32(np, "kionix,accel-g-range", &temp_val);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read accel-g-range\n");
		return rc;
	} else {
		switch (temp_val) {
		case 2:
				 ...
				 const struct i2c_device_id *id)
{
	...
	accel_pdata = devm_kzalloc(&client->dev, sizeof(struct kionix_accel_platform_data),
			GFP_KERNEL);
	if (!accel_pdata) {
	...
	if (client->dev.of_node) {
		//memset(accel_pdata, 0 , sizeof(accel_pdata));
		...
		if (client->dev.platform_data)
			accel_pdata = (struct kionix_accel_platform_data *)
	...
	kionix_accel_vdd_power_init(acceld, true);
	kionix_accel_vdd_power_ctl(acceld, true);
	err = kionix_accel_power_on(acceld);
	if (err < 0)
		goto err_free_mem;
	...
	if (proc_dir == NULL)
		...
		proc_entry = proc_mkdir_mode( "sensors/accelinfo", 0644, NULL);
		if (proc_entry == NULL)
	...
	acceld->cdev.min_delay = POLL_INTERVAL_MIN * 1000;
	acceld->cdev.delay_msec = acceld->poll_delay;
	acceld->cdev.sensors_enable = kionix_accel_enable_set;
	acceld->cdev.sensors_poll_delay = kionix_accel_poll_delay_set;
	err = sensors_classdev_register(&acceld->client->dev, &acceld->cdev);
	if (err) {
		dev_err(&acceld->client->dev, "create class device file failed!\n");
		err = -EINVAL;
...
err_accel_pdata_power_off:
	kionix_accel_power_off(acceld);
	kionix_accel_vdd_power_init(acceld, false);
err_free_mem:
	kfree(acceld);
	return err;
}
...
#else
#define kionix_match_table NULL
#endif
