...
#define ALS_debug       1		/* Increase to 2 for more details */
#define ALS_info        1
#define ALS_err         1
	...
	if (enable) {
		...
		if (ret >= 0) {
			if (!ps_data->suspended) {
				...
				printk(KERN_ERR "%s: Doing an enabled while suspended!\n", __func__);
			}
		}
	} else {
...
static int ap3426_als_enable(struct ap3426_data *ps_data, int enable)
{
	...
	if (ps_data->als_re_enable) {
		/* NOTE: Need to enable/disable timer below if re-enabling ALS */
		ALS_DBG("Enable/Disable ALS even if it's currently enabled/disabled.\n");

		ALS_DBG("ps_data->als_re_enable:%d = 0;\n", ps_data->als_re_enable);

		ps_data->als_re_enable = 0;
	}
	else if (misc_als_opened == enable) {
