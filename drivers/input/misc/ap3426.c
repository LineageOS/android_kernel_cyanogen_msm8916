...
/*
 * Uncomment the 3 lines within the #ifndef...#endif to enable pr_debug()
...
// #define ALS_DEBUG                    /* Uncomment for additioal ALS Debug */
 ...
 * where it appears the debug code is enabked for a CONFIG_DYNAMIC_DEBUG kernel.
 *
...
static int ap3426_get_adc_value(struct i2c_client *client)
{
    ...
    ALS_DBG("tmp:%d = (((msb:%d << 8) | lsb:%d) * range:%d) >> 16;\n",
             tmp,        msb,           lsb,      range);

    tmp2 = tmp * cali / 100;
    ALS_DBG("tmp2:%d = tmp:%d * cali:%d\n",
             tmp2,     tmp,     cali);
#endif
...
static int ap3426_get_object(struct i2c_client *client)
{
    ...
    PS_DBG("val:0X%x &= AP3426_OBJ_MASK:0x%x\n", val, AP3426_OBJ_MASK);
    val &= AP3426_OBJ_MASK;

    rv = !(val >> AP3426_OBJ_SHIFT);
    PS_DBG("rv = 0x%x = !(val:0x%x >> AP3426_OBJ_SHIFT):%d;\n",
             rv,          val,        AP3426_OBJ_SHIFT);
...
static int ap3426_ps_enable(struct ap3426_data *ps_data, int enable)
{
    ...
    if (ret < 0) {
        PS_ERR("ret:%d = ap3426_power_on/off_ps(); Error\n", ret);
    } else {
...
static int ap3426_ls_enable(struct ap3426_data *ps_data,int enable)
{
        ...
        ALS_DBG("misc_ls_opened = %d, enable=%d\n", misc_ls_opened, enable);
...
static int ap3426_set_crosstalk_calibration(struct i2c_client *client, int val)
{
    int lsb, msb, err;
...
int ap3426_calibration(struct i2c_client *client)
{
    ...
    if (!ps_calibrated) {
        ap3426_set_crosstalk_calibration(client, 0);
       ...
       if (samples >= CAL_SAMPLES/2) {
		...
		samples++;
            ...
            ave = total/samples;
            PS_DBG("ave = %d \n", ave);
            ap3426_set_crosstalk_calibration(client, ave);
            msleep(50);
...
void pl_timer_callback(unsigned long pl_data)
{
    ...
    if (ret) {
        ALS_DBG("ret = %d = mod_timer(...); ret, [Timer Error?]\n", ret);
    }
    ALS_RETURN_VOID();
    return;
}
...
static int ap3426_parse_dt(struct device *dev, struct ap3426_data *pdata)
{
    ...
    if (!rc) {
        ...
        ALS_DBG("ps-calibration_expected %d\n", pdata->ps_calibration_expected);
    } else {
...
static int ap3426_resume(struct device *dev)
{
    ...
    ENTRY("dev:%p)", dev);
    DBG("ps_data:%p->rels_enable:%d\n", ps_data, ps_data->rels_enable);
