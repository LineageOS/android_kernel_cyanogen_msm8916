...
static int ap3426_als_enable(struct ap3426_data *ps_data, int enable)
{
	...
	if (enable && ps_data->suspended) {
		printk(KERN_ERR "%s: Enable while Suspended!\n", __func__);
	}
	if (ps_data->als_re_enable) {
		LDBG("Enable/Disable ALS even if it's currently enabled/disabled.\n");

		LDBG("ps_data->als_re_enable:%d = 0;\n",
		      ps_data->als_re_enable);

		ps_data->als_re_enable = 0;
	}
	else if (misc_als_opened == enable) {
		 LDBG("misc_als_opened:%d == enable:%d and !als_re_enable; Done.", \
		       misc_als_opened,      enable);
...
static int ap3426_suspend(struct device *dev)
{
	...
	LDBG("ps_data->als_re_enable:%d = 1;\n",  ps_data->als_re_enable);
...
static int ap3426_resume(struct device *dev)
{
	...
	ap3426_als_enable(ps_data, true);
