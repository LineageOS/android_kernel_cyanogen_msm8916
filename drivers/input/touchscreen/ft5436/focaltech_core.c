...
#if FTS_GESTRUE_EN
extern int fts_gesture_enabled;   //add by liuyang,20160521
#endif

//Start:reqxxx,liuyang3.wt,ADD,20160521,for golve switch.
#if FTS_GLOVE_SURPORT
#define FTS_GLOVE_SWITCH_PROC_FILE "goodix_glove_switch"
static ssize_t fts_glove_switch_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t fts_glove_switch_write_proc(struct file *, const char __user *, size_t, loff_t *);
...
static const struct file_operations glove_switch_proc_ops = {
    ...
    .write = fts_glove_switch_write_proc,
};

static ssize_t fts_glove_switch_read_proc(struct file * filp, char __user *buffer,  size_t count, loff_t * off)
{
	s32 ret = -1;
	char buf[20] = {0};
...
static ssize_t fts_glove_switch_write_proc(struct file * filp, const char __user * buffer, size_t count, loff_t * off)
{
	int ret = 0;
	char  temp[32] = {0};
	...
	ret = kstrtouint(temp, 0, &fts_glove_enabled);
	if (ret){
		pr_err("glove_enabled = %d, ret:%d \n", fts_glove_enabled, ret);
		return -EFAULT;
	}
	pr_info("temp = %s, glove_enabled = %d\n",temp, fts_glove_enabled);

	if( fts_glove_enabled ){		
    		ret = fts_write_reg(fts_i2c_client, 0xC0, 1);// open glove function.
    		if (ret < 0)
    		{
     		   pr_err("send glove config failed.");
    		}	
	}else {
    		ret = fts_write_reg(fts_i2c_client, 0xC0, 0); //close glove function.
    		if (ret < 0)
    		{
     		   pr_err("send glove config failed.");
    		}		
...
#endif
//End:Reqxxx,liuyang3.wt,ADD,20160521,for glove switch.
...
//Start:Reqxxx,liuyang3.wt,Modify,20160512, for report KEY event.
#if CTP_HAVE_KEYS
			if(event->au16_y[i]>=1310 && event->au16_y[i] <= 1390)/*modified by anhengxuan 2016.6.02,center_y(1350)\A1\C040*/
			{
				key_flag=1;
				if (event->au16_x[i]>=60&&event->au16_x[i] <= 180)/*modified by anhengxuan 2016.6.02,center_key_back(120)\A1\C060*/
				{
					key_value = KEY_BACK;
					input_report_key(data->input_dev, KEY_BACK, 1);
				}
				else if(event->au16_x[i]>=300 && event->au16_x[i] <= 420)/*modified by anhengxuan 2016.6.02,center_key_home(360)\A1\C060*/
				{
					key_value = KEY_HOME;
					input_report_key(data->input_dev, KEY_HOME, 1);
				}else if(event->au16_x[i]>=580 && event->au16_x[i] <= 640)/*modified by anhengxuan 2016.6.02,center_key_menu(580)\A1\C060*/
				{
					key_value = KEY_MENU;
...
//End:Reqxxx,liuyang3.wt,Modify,20160512, for report KEY event.			
...
int fts_ts_suspend(struct device *dev)
{
	...
	int err = 0;
	pr_info("%s********\n", __func__);
//Start:Reqxxx,liuyang3,wt,Modify,for add gesture switch.
#if FTS_GESTRUE_EN
	if (fts_gesture_enabled)
	{
		  fts_write_reg(fts_i2c_client, 0xd0, 0x01); //enter minitor mode.
		  pr_info("%s fts:enter minitor mode.\n", __func__);
...
#endif
//End:Reqxxx,liuyang3,wt,Modify,for add gesture switch.
	if (data->loading_fw) {
...
#endif
//add by liuyang,30160521
#if FTS_GESTRUE_EN
	if (fts_gesture_enabled)
	{
		fts_write_reg(fts_i2c_client, 0xd0, 0x00); //exit minitor mode.
		data->suspended = false;
		err = disable_irq_wake(data->client->irq);
		if (err)
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	pr_info("%s %d pdata->irq_gpio = %d, irq_gpio_flags = %u\n", __func__, __LINE__, pdata->irq_gpio,pdata->irq_gpio_flags) ;
	if (pdata->irq_gpio < 0)
	{
...
//End:Reqxxx,liuyang3.wt,Modify,20160512, for report KEY event.	
	...
	msleep(data->pdata->soft_rst_dly);
	pr_info("soft_rst_dly= %u\n",data->pdata->soft_rst_dly );
	...
	data->family_id = pdata->family_id; //family_id???
...
//Start:modify by liuyang3,20160521
	#if FTS_GESTRUE_EN
		fts_Gesture_init(input_dev);
		err = fts_gesture_init_node();
		if (err < 0)
		{
			dev_err(&client->dev, "fts_gesture_init_node fail!\n");
		}
	#endif
//End
//modify for firware update.	
...
//add by liuyang,20160521
#if FTS_GESTRUE_EN
		fts_gesture_deinit_node();
#endif

#if FTS_GLOVE_SURPORT
		remove_proc_entry(FTS_GLOVE_SWITCH_PROC_FILE, NULL);
#endif
