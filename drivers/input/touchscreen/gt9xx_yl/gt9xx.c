...
/* Close TW_DEBUG; added by litao3; 140102
#define YL_TW_DEBUG */
...
/* Transplant TW GTP_ESD_PROTECT code from 8675_C00
 * Add declaration of watchdog and esd switch functions
 * By liushilong@yulong.com on 2014-11-6 16:57
 */
...
enum support_gesture_e {
	TW_SUPPORT_NONE_SLIDE_WAKEUP = 0x00,
	TW_SUPPORT_UP_SLIDE_WAKEUP = 0x01,
	TW_SUPPORT_DOWN_SLIDE_WAKEUP = 0x02,
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	if (retries >= 5) {
#if GTP_SLIDE_WAKEUP
		/* Resetting the chip would quit doze mode */
		if (doze_status == DOZE_ENABLED)
			return ret;
#endif
...
{
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
	if (in_hall) {    /* Detected Hall leave-state */
...
#ifdef CONFIG_TOUCHSCREEN_GT9XX_YL_COVER_WINDOW_SIZE
if (atomic_read(&gt9xx_cover_window.windows_switch)
...
#else
/*
	Use MT-A protocol for Android 4.0
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	u8 point_data[4 + 8 * GTP_MAX_TOUCH] =
...
#endif
#if GTP_SLIDE_WAKEUP
	/* For testing, added by yewenliang */
	...
	*/
	...
	if (touch_num) {
		for (i = 0; i < touch_num; i++) {
			...
			if (pre_touch < touch_num &&
				pre_touch_status ^ (0x01 << id)) {
					GTP_INFO("Point[%d] Touch Down! x=%d, y=%d.",
							id, input_x, input_y);
			}
#endif
		}
...
static s8 gtp_enter_sleep(struct goodix_ts_data *ts)
{
	...
	/* For power consume, by qysh on 11.02
...
s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	if (ts->gtp_cfg_len < GTP_CONFIG_MIN_LENGTH) {
		GTP_ERROR("Sensor_ID(%d) matches with NULL or INVALID CONFIG GROUP! NO Config Sent! You need to check you header file CFG_GROUP section!",
				sensor_id);
		return -1;
	}
	...
	if (ret == SUCCESS &&
		opr_buf[0] < 90 &&
		/* Change the condition by that not equal */
...
/* endif GTP_CUSTOM_CFG */
	...
	if (cfg_version_bak) {
		...
		msleep(10);
		ret = gtp_send_cfg(ts->client);
		msleep(500);   /* Sleep for 500 msec to update completely */
	}
#endif
...
static s32 gtp_write_config(struct goodix_ts_data *ts)
{
	...
	if (glove_mode == 1) {
		...
		memcpy(&config[GTP_ADDR_LENGTH],
				yl_cfg_glove[tp_index].firmware, glove_gtp_cfg_len);
	} else if(glove_mode == 0) {
		glove_gtp_cfg_len = yl_cfg[tp_index].firmware_size;
		memset(&config[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
		memcpy(&config[GTP_ADDR_LENGTH],
				yl_cfg[tp_index].firmware, glove_gtp_cfg_len);
	}
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
	...
	if (buf[5] == 0x00)
		printk("IC Version: %c%c%c_%02x%02x",
...
#if GTP_ICS_SLOT_REPORT
	input_mt_init_slots(ts->input_dev, 10);	/* In case of OOM situations */
#else
	/* Use the MT-A protocol for Android 4.0 */
...
int goodix_firmware_need_update(void)
{
	...
	GTP_INFO("Checking if afirmware update is needed...\n");
	GTP_DEBUG("Enter %s\n",__FUNCTION__);

	ret = gup_get_ic_fw_msg(i2c_connect_client);
	if (ret == FAIL) {
		GTP_ERROR("[update_proc] Failed to get ic message");
		return FAIL;
	}
...
int goodix_get_firmware_version(char * version )
{
	...
	if (tp_supported)
		return sprintf(version, "%s:%s:0x%4X:0x%2X", yl_cfg[tp_index].tp_name,
				TW_IC_PREFIX_NAME, gtp_fw_version, gtp_cfg_version);
	else
		return sprintf(version, "%s:%s:0x%4X:0x%2X", "----",
				TW_IC_PREFIX_NAME, gtp_fw_version, gtp_cfg_version);
}
...
/* TW_GLOVE_SWITCH interface, added by liushilong@yulong.com, 2014-11-6 17:15 */
...
int goodix_read_regs(char *buf)
{
	...
	cfg[0] = GTP_REG_CONFIG_DATA >> 8;
	cfg[1] = GTP_REG_CONFIG_DATA & 0xff;
	ret = gtp_i2c_read(i2c_connect_client, cfg,
			GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
	if (ret < 0) {
		GTP_ERROR("Read config version failed.\n");
		return ret;
...
int goodix_gesture_ctrl(const char *gesture_buf)
{
	...
	char *gesture_p;
	char *temp_p;
	/* Added by longjiang on 2014-06-04 */
	s8 ret = -1;
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
	...
	while (gesture_p) {
		...
		if (!strncmp(gesture, "c=", 2)) {
			if(!strncmp(gesture + 2, "true", 4)) {
				...
				GTP_DEBUG("%s: disable cC click wakeup func.\n", __func__);
				support_gesture &= ~TW_SUPPORT_C_SLIDE_WAKEUP;
			}
			continue;
		}
	...
	/* Added by longjiang on 2016-06-04 */
	...
	if (support_gesture & TW_SUPPORT_GESTURE_IN_ALL) {
		...
		/* Evaluate doze status before entering doze mode.
		   added by liushilong@yulong.com, 2014-11-6 17:15 */
		mutex_lock(&ts->doze_mutex);
						...
						bool on)
{
	...
	pins_state = on ? ts->gpio_state_active : ts->gpio_state_suspend;
...
/* TW GTP_ESD_PROTECT code from 8675_C00, reset init watchdog,
   added by liushilong@yulong.com on 2014-11-6 17:24 */
#if GTP_ESD_PROTECT
...
static int goodix_ts_parse_dt(struct device *dev, struct tw_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "goodix,irq_flags",
				(unsigned int *) &pdata->irqflag);
	if (ret) {
...
static ssize_t glove_store(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t count)
{
	...
	switch (glove_switch) {
	/* Not charging; opened glove mode */
	case 0:
		glove_mode = 1;
		gtp_write_config(ts);
		GTP_INFO("Not charging, opened glove mode, glove config write over!");
		break;
	...
	case 1:
		...
		break;
	/* Charging; opened glove mode first,
	   then enter finger mode, then exit glove mode */
	case 2:
		i2c_control_buf[2] = 0x0A;
		i2c_control_buf[0] = 0x80;
		i2c_control_buf[1] = 0x46;
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		GTP_INFO("%s to set finger mode flag into 0x8046\n",
				ret < 0 ? "Failed" : "Succeeded");
		i2c_control_buf[0] = 0x80;
		i2c_control_buf[1] = 0x40;
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		if (ret > 0)
			GTP_INFO("charger in, opened glove mode. GTP enter finger mode!");
		else
			GTP_INFO("charger in, opened glove mode. Write register 0x8040 0x0A err!");
		break;
	/* Ended charging; opened glove mode */
	case 3:
		i2c_control_buf[2] = 0x0B;
		i2c_control_buf[0] = 0x80;
		i2c_control_buf[1] = 0x46;
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		GTP_INFO("%s to set automatic switching mode flag into 0x8046\n",
				ret < 0 ? "Failed" : "Succeeded");
		i2c_control_buf[0] = 0x80;
		i2c_control_buf[1] = 0x40;
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		if (ret > 0)
			GTP_INFO("Ended charging, opened glove mode, GTP enter glove mode!");
		else
			GTP_INFO("Ended charging, opened glove mode, Write register 0x0B err!");
		break;
	case 4:
		GTP_INFO("4 charging, glove mode was closed, nothing to do!");
		break;
	case 5:
		GTP_INFO("5 charging exit, glove mode was closed, nothing to do!");
		break;
	/* First charging, then open glove mode,
	   then enter finger mode, and lastly, close glove mode */
	case 6:
		glove_mode = 1;
		i2c_control_buf[2] = 0x0A;
		gtp_write_config(ts);
		GTP_INFO("Charging first, then open glove mode, glove config write over!");
		i2c_control_buf[0] = 0x80;
		i2c_control_buf[1] = 0x46;
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		GTP_INFO("%s to set finger mode flag into 0x8046\n",
		...
		ret = gtp_i2c_write(ts->client, i2c_control_buf, 3);
		if (ret > 0)
			GTP_INFO("charging first, then open glove mode, write 0x8040 0A, GTP enter finger mode!");
		else
			GTP_INFO("charging first, then open glove mode, write register 0A err!");
		break;
	/* Exit glove mode; now charging */
	case 7:
		glove_mode = 0;
		gtp_write_config(ts);
		GTP_INFO("Exit glove mode, now charging: send finger_config.");
		break;
	default:
...
static ssize_t gt968_send_cfg_ver_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	u8 ret = 0;
...
/****************************config send************/
static ssize_t gt968_send_cfg_ver_store(struct device *dev,
...
/* Small window cover. Added by linronghui&mengxiuguan on2014.02.11 */
#ifdef CONFIG_TOUCHSCREEN_GT9XX_YL_COVER_WINDOW_SIZE
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	} else if (!atomic_read(&gt9xx_cover_window.windows_switch)
			&& val == 1) {
...
static DEVICE_ATTR(glove_switch,
		...
		glove_store);
#ifdef CONFIG_TOUCHSCREEN_GT9XX_YL_COVER_WINDOW_SIZE
static DEVICE_ATTR(windows_switch,
		S_IRUGO | S_IWUSR,
		windows_show,
		windows_store);
#endif
...
/* Small window cover. Added by linronghui&mengxiuguan on 2014.02.11 */
#ifdef CONFIG_TOUCHSCREEN_GT9XX_YL_COVER_WINDOW_SIZE
	...
	/* Do NOT remove these output logs */
	...
	/* Initialize doze mode mutex, by liushilong@yulong.com, 2014-11-6 17:28 */
...
/* Transplant TW GTP_ESD_PROTECT code from 8675_C00,
    set clock click count for esd check circle,
    by liushilong@yulong.com on 2014-11-6 17:30 */
#if GTP_ESD_PROTECT
...
/* Remove these functions;
   these should be work on initialize period;
   delete 5 lines;
   by liushilong@yulong.com on 2014-11-6 17:40 */
//#if GTP_ESD_PROTECT
...
/* Destroy the doze_mutex if exit_init_failed,
	by liushilong@yulong.com on 2014-11-6 17:46 */
...
#if GTP_ESD_PROTECT
	ts->gtp_is_suspend = 1;
/* Switch the ESD switch off when gtp is suspended,
	by liushilong@yulong.com on 2014-11-06 17:47 */
...
//	}
	...
	/* Add mutex for doze mode, liushilong@yulong.com on 2014-11-6 17:49 */
	mutex_lock(&ts->doze_mutex);
...
/* Modify delayed work instead of gtp_esd_switch;
	it should switch on esd protct when late resume,
	by liushilong@yulong.com on 2014-11-6 17:49 */
#if GTP_ESD_PROTECT
...
/* Transplant TW GTP_ESD_PROTECT code from 8675_C00:
	When esd test is fail, it should restart the mobile phone
	...
	Modify: gtp_esd_check_func()

	By liushilong@yulong.com on 2014-11-7 14:00 */
#if GTP_ESD_PROTECT
...
/* Transplant TW GTP_ESD_PROTECT code from 8675_C00:
	When initialize goodix touch panel, it should INIT and check esd.
	By liushilong@yulong.com on 2014-11-6 17:24 */
#if GTP_ESD_PROTECT
	INIT_DELAYED_WORK(&gtp_esd_check_work, gtp_esd_check_func);
	gtp_esd_check_workqueue = create_workqueue("gtp_esd_check");
#endif
