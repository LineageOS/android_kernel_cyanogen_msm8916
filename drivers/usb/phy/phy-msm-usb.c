...
static int otg_enable(struct msm_otg_platform_data *pdata, bool enable)
{
	...
	gpio_direction_output(pdata->otg5v_en, enable);
...
#ifdef CONFIG_MACH_JALEBI
	pdata->otg5v_en = of_get_named_gpio(node, "qcom,otg5v_en-gpio", 0);
	if (pdata->otg5v_en < 0)
		pr_debug("otg5v_en is not available\n");
	dev_info(&pdev->dev, "get otg5v_en = %d\n", pdata->otg5v_en);
#endif
...
#ifdef CONFIG_MACH_JALEBI
		if (gpio_is_valid(motg->pdata->otg5v_en)) {
			/* otg5v_en_gpio request */
			ret = gpio_request(motg->pdata->otg5v_en, "OTG5V_EN");
			if (ret < 0) {
				dev_err(&pdev->dev, "otg5v_en gpio req failed for id\n");
				motg->pdata->otg5v_en = 0;
				goto remove_phy;
			} else {
				dev_info(&pdev->dev, "otg5v_en gpio req success for id\n");
			}
		}
#endif
	}
