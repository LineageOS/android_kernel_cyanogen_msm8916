		...
		int method, unsigned char *obuf, size_t olen)
{
	...
	while (ilen > 0) {
		...
		if (!page) {
			printk(KERN_INFO "%s: page went away\n", __func__);
			continue;
		}
		pageaddr = kmap_atomic(page);
...
static void cluster_wait(struct work_struct *work)
{
	...
	while (len > 0) {
		...
		wait_on_page_locked(page);
		if (PageError(page)) {
			/* XXX: set error on upper pages, unlock and release compressed pages */
			/* XXX: wait on all pages? */
	...
	obuf = vmalloc(olen);
	if (!obuf) {
		/* XXX: set error on upper pages, release compressed pages */
		printk(KERN_INFO "%s: out of memory\n", __func__);
		goto out_err;
	}

	cluster_decompress(clst, &info->i_mapping, XCOMP_METHOD(info->i_param1), obuf, olen);
